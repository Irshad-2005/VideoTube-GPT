...
const Login = () => {
    ...
    .setProject("67bdf9b10001d5d2c26b");
    ...
    const handleSignInSignUpForm = ()=>
    {
        ...
        {
            //sign up form
        const user = account.create(ID.unique(),email.current.value,password.current.value,name.current.value);
        user.then((resp)=>
        {
            console.log(resp);
        }).catch((error)=>
        {
            ...
            //sign in form
            const user = account.createEmailPasswordSession(email.current.value, password.current.value);
            user.then((resp)=>
            {
    ...
    return (
        <div >
            <Header/>
            <div className='w-full h-screen absolute a'>
